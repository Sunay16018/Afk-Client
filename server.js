...
function startBot(sid, host, user, ver) {
    ...
    bot.on('login', () => logs[key].push("<b style='color:#2ecc71'>[SİSTEM] Bot Giriş Yaptı!</b>"));
...
http.createServer((req, res) => {
    ...
    if (p === '/update' && bot) {
        ...
        if (q.type === 'msg') {
            ...
            if (q.status === 'on') {
                conf.msgT = setInterval(() => bot.chat(decodeURIComponent(q.val)), q.sec * 1000);
            }
        } 
        // ANTI-AFK
        else if (q.type === 'afk') {
            ...
            if (q.status === 'on') {
                conf.afkT = setInterval(() => {
                    bot.look(Math.random() * 6.2, (Math.random() - 0.5));
